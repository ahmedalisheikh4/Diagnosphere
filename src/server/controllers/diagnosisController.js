...
const submitSymptoms = async (req, res) => {
  try {
    ...
    if (diagnosis.user.toString() !== req.userId.toString()) {
      return res
        .status(403)
        .json({ message: "Not authorized to access this diagnosis" });
    }
    ...
    diagnosis.results = {
      predictions: [
        ...
        { name: "Psoriasis", probability: 0.2 },
    ...
    if (!updatedDiagnosis) {
      return res
        .status(500)
        .json({ message: "Failed to save symptoms and results" });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error during symptoms submission" });
  }
};
